...
- (BOOL)prehandle:(BeeMessage *)msg
{
    ...
    if ( msg.succeed )
    {
		...
		if ( status.error_code.integerValue == 100 || status.error_code.integerValue == 1 )
        {
            [[UserModel sharedInstance] setOffline:YES];
            return NO;
        }
    }
...
//		NSString * requestURI = @"http://shop.ecmobile.me/ecmobile/?url=order/pay";
...
DEF_MESSAGE_( price_range, msg )
{
	if ( msg.sending )
	{
        ...
        //		NSString * requestURI = @"http://shop.ecmobile.me/ecmobile/?url=price_range";
